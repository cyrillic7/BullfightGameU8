...
void NDKHelper::AddSelector(const char *groupName, const char *name, SEL_CallFuncND selector, CCObject* target)
{
...
void NDKHelper::HandleMessage(json_t *methodName, json_t* methodParams)
{
    ...
    for (unsigned int i = 0; i < selectorList.size(); ++i)
    {
        if (selectorList[i].getName().compare(methodNameStr) == 0)
        {
            ...
            SEL_CallFuncND sel = selectorList[i].getSelector();
            CCObject *target = selectorList[i].getTarget();
            
			CCCallFuncND *callfunc = CCCallFuncND::create(target, sel, (void*)dataToPass);
			callfunc->execute();

			//callfuncs->addObject(CCCallFuncND::create(target, sel, dataToPass));
			//CCDirector::sharedDirector()->getScheduler()->scheduleSelector(callfunc_selector(NDKHelper::ExecuteCallfuncs), this, 0, 0, 0, false);
...
extern "C"
{
	...
	std::string getMessageWithParams(std::string methodName, CCObject* methodParams) {
		std::string strInfo = "";
		...
		bool isHave = JniHelper::getStaticMethodInfo(t,
			...
			"()Ljava/lang/String;");
		if (isHave) {
			CCLog("jni->%s/getChannel:is exit", CLASS_PAGE);
			//char *jsonStrLocal = json_dumps(toBeSentJson, JSON_COMPACT | JSON_ENSURE_ASCII);
			...
			CCLog("getMessageWithParams  %s", strInfo.c_str());
		...
		{
			CCLog("jni->%s/getChannel:not exit", CLASS_PAGE);
		}
#endif
